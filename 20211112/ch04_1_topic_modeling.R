...
news_comment <- raw_news_comment %>%
  mutate(reply = str_replace_all(reply, "[^가-힣]", " "),
         reply = str_squish(reply)) %>% #공백제거
  distinct(reply, .keep_all = TRUE) %>% # 중복 댓글 제거
  filter(str_count(reply, boundary("word")) >= 3) # # 짧은 문서 제거 - 3 단어 이상 추출

# 명사 추출
comment <- news_comment %>%
  unnest_tokens(input = reply, #tidytext  function
                output = word, 
                token = extractNoun,
                drop = F) %>%
